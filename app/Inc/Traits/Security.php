...
namespace HQFW\Inc\Traits;

defined( 'ABSPATH' ) || exit;

/**
 * The `Security` class provides a method that can be used in checking
 * security or validating `$_POST` during an AJAX or https request.
 *
 * @since 1.0.0
 */
trait Security {
     ...
     *
     * @param  array $post Contains the $_POST data.
     * @param  array $keys Contains the list of keys in post.
     * @return boolean
     */
    private static function has_post_empty_data( $post, $keys = array() ) {
        if ( $keys ) {
            foreach ( $keys as $key ) {
                if ( empty( $post[ $key ] ) ) {
                    return true;
